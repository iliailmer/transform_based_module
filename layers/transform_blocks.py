...
class TransformBlock(nn.Module):
    def __init__(self, input_channels, output_channels,
        ...
        if add_noise:
            self.noise = nn.Parameter(nn.init.normal_(
        ...
        if lmbda:
            ...
            return torch.stack(
                tuple([(filter_bank.view(-1, 1,
                                         ...
                                         kernel_size))[ids, ...]] *
                      input_channels),
                dim=0).view((-1, 1, kernel_size, kernel_size))
        if diag:
            ids = self.get_idx_diag(kernel_size)
            return torch.stack(
                tuple([filter_bank.view(-1,
                                        1,
                                        kernel_size,
                                        kernel_size)[ids, ...]] *
                      input_channels),
...
class HarmonicBlock(TransformBlock):
    def __init__(self, *args, type=2, droprate=0.5, **kwargs, ):
        super(HarmonicBlock, self).__init__(*args, **kwargs)
        self.type = type
        self.filter_bank = self\
            .get_filter_bank(kernel_size=self.kernel_size,  # kernel size
                             input_channels=self.input_channels,
                             t=self.type,  # type of DCT
                             lmbda=self.lmbda,
                             diag=self.diag).float()
        if self._cuda:
    ...
    def filter_from_matrix(self, i, j, size):
        ...
        fltr = mat[i, :self.kernel_size].reshape(
            (-1, 1)).dot(mat[j, :self.kernel_size].reshape(1, -1))
    ...
    def forward(self, x):
        ...
        x = F.conv2d(x.float(),
                     ...
                     stride=self.stride,
                     groups=self.input_channels)  # int(self.K/
            ...
            if N == 2:
                ...
                while i >= 3 and i <= n:
                    ...
                    S_N = np.block([[An1,
                                    ...
                                    [Bn1, np.zeros((2, N // 2 - 2)),
                                     Bn2, np.zeros((2, N // 2 - 2))],
...
class ResNextTransofrmBlock(nn.Module):
    ...
    def __init__(self, input_channels,
        ...
        self.conv0 = transform_conv(input_channels,
                                    kernel_size=3,
                                    padding=1,
                                    stride=1,
                                    lmbda=2)
