...
class TransformWRN(nn.Module):
    def __init__(self, in_channels,
        ...
        if not isinstance(block, list):
            ...
            assert len(
                block) == 4, f"Length of block list must be 4, {len(block)}"
        self.block = block
        if not isinstance(alpha_root, list):
            ...
            if len(alpha_root) == 1:
                ...
                assert len(
                    alpha_root) == 4, f"Length of alpha root list must be 4" +\
        ...
        if not isinstance(add_noise, list):
            ...
            if len(add_noise) == 1:
                ...
                assert len(
                    add_noise) == 4, f"Length of alpha root list must be 4," +\
                                     f" {len(add_noise)}"
        ...
        if not isinstance(kernel_size, list):
            ...
            if len(kernel_size) == 1:
                ...
                assert len(
                    kernel_size) == 4, f"Length of kernel size list " +\
        ...
        if block[0] == BasicBlock or block[0] == Bottleneck:
            ...
            self.conv1 = block[0](input_channels=in_channels,
        ...
        self.stack1 = self._make_layer(block[1],
                                       nb_layers=n[1],
        ...
        self.stack2 = self._make_layer(block[2],
                                       nb_layers=n[2],
        ...
        self.stack3 = self._make_layer(block[3],
                                       nb_layers=n[3],
    ...
    def _make_layer(self, block,
                    in_planes, out_planes, alpha_root,
    ...
    def _num_parameters(self, trainable=True):
        ...
        for m in self.modules():
            ...
            if isinstance(m, HarmonicBlock) or \
                isinstance(m, HadamardBlock) or \
                    isinstance(m, SlantBlock):
...
class TransformResNet(nn.Module):
    def __init__(self, in_channels,
        ...
        nChannels = [out_channels, out_channels,
        ...
        if not isinstance(kernel_size, list):
            ...
            if len(kernel_size) == 1:
                ...
                assert len(
                    kernel_size) == 4, f"Length of kernel size list must be" +\
                    f" 4, {len(kernel_size)}"
                kernel_size = kernel_size
        self.kernel_size = kernel_size
        pad = [k // 2 for k in kernel_size]
        ...
        if block[0] == BasicBlock or block[0] == Bottleneck:
            ...
            self.conv1 = block[0](input_channels=in_channels, bn=self.bn,
                                  dropout=self.dropout,
        ...
        self.stack0 = self._make_layer(block[0],
                                       nb_layers=n[0],
...
class ResNeXt(nn.Module):
    def __init__(self, Block, num_blocks, transform_conv,
                 cardinality, bottleneck_width, num_classes=10):
    ...
    def _make_layer(self, num_blocks, transform_conv, stride):
        ...
        for stride in strides:
            layers.append(self.block(self.in_planes, self.cardinality,
                                     transform_conv,
                                     self.bottleneck_width, stride))
            self.in_planes = self.block.expansion *\
